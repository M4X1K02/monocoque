#!/bin/bash
# Assert a source install produced the files play mode needs.
...
forbid_grep() {
    ...
    local pattern="$2"
    if grep -Eq -- "$pattern" "$path"; then
        log "FAIL $path still contains $pattern"
        FAIL=$((FAIL + 1))
    else
        log "PASS $path does not contain $pattern"
        PASS=$((PASS + 1))
    fi
}
...
forbid_grep "$BIN_DIR/start-monocoque" 'Start order'
